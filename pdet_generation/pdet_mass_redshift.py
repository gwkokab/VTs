...
sensitivity = ls.SimNoisePSDaLIGOEarlyHighSensitivityP1200087  # Power spectral density model
...
def optimal_snr(
    ...
    tmax = (
        ls.SimInspiralChirpTimeBound(
            fmin, m1z * lal.MSUN_SI, m2z * lal.MSUN_SI, 0.0, 0.0
        )
        + 2.0
    )
    ...
    hp, _ = ls.SimInspiralChooseFDWaveform(
        ...
        0.0,
        0.0,
        0.0,  # spins in dimensionless units
...
def fraction_above_threshold(
    ...
    rho_max = optimal_snr(
        m1, m2, z, fmin, dfmin, fref, psdstart, psd_fn, approximant
    )
...
def pdets_from_masses_spins(
    ...
    args = [
        (m1, m2, z, thresh, 19.0, 0.0, 40.0, 20.0, psd_fn, waveform)
...
def main():
    ...
    pdets = pdets_from_masses_spins(m1s, m2s, zs, 8.0)
    ...
    with h5py.File(output, "w") as f:
        ...
        f.attrs["description"] = (
            ...
            "with uniform priors in mass and redshift. "
            "The pdet values are computed using the IMRPhenomPv2 waveform model "
            "and the SimNoisePSDaLIGOEarlyHighSensitivityP1200087 sensitivity model. "
            f"The redshift is uniformly sampled from 0 to {params['redshift_max']}."
        )

        f.attrs["waveform"] = "IMRPhenomPv2"
        f.attrs["sensitivity"] = "SimNoisePSDaLIGOEarlyHighSensitivityP1200087"
        f.create_dataset(
            "names",
            data=np.array(
                [
