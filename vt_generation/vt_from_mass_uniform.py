...
def main():
    ...
    output = "./vt_0p5_200_IMRPhenomPv2_SimNoisePSDaLIGO175MpcT1800545.hdf5"

    with h5py.File(output, "w-") as f:
        # we can calculate the VT without using the grid
        m1s = np.random.uniform(0.5, 200, 100000)
        m2s = np.random.uniform(0.5, 200, 100000)
    ...
    plt.hist2d(m1s, m2s, weights=vts, bins=80, cmap="viridis")
    plt.xlabel("m1")
    plt.ylabel("m2")
    plt.title("Analytical VT")

    plt.savefig("uni_vt.pdf")
